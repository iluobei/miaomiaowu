...
func (p *ShadowrocketProducer) produceProxiesOnly(proxies []Proxy, outputType string, opts *ProduceOptions) (interface{}, error) {
...
func (p *ShadowrocketProducer) produceFullConfig(proxies []Proxy, opts *ProduceOptions) (string, error) {
	var sb strings.Builder
...
func (p *ShadowrocketProducer) generateGeneral(opts *ProduceOptions) string {
	var sb strings.Builder
...
func (p *ShadowrocketProducer) extractDNSServers(fullConfig map[string]interface{}) []string {
	var servers []string
...
func (p *ShadowrocketProducer) extractIPv6Config(fullConfig map[string]interface{}) bool {
	if dnsConfig, ok := fullConfig["dns"].(map[string]interface{}); ok {
...
func (p *ShadowrocketProducer) generateProxies(proxies []Proxy, opts *ProduceOptions) string {
	var sb strings.Builder
...
func (p *ShadowrocketProducer) formatProxyLine(proxy Proxy) string {
	proxyType := GetString(proxy, "type")
	name := GetString(proxy, "name")
...
func (p *ShadowrocketProducer) generateProxyGroups(opts *ProduceOptions) string {
	var sb strings.Builder
...
func (p *ShadowrocketProducer) formatProxyGroupLine(group map[string]interface{}) string {
...
func (p *ShadowrocketProducer) generateRules(opts *ProduceOptions) string {
	var sb strings.Builder
	sb.WriteString("[Rule]\n")
	sb.WriteString("# 规则配置\n")
...
func (p *ShadowrocketProducer) formatRuleLine(rule string, ruleProviders map[string]string) string {
...
func (p *ShadowrocketProducer) generateHost() string {
	return "[Host]\nlocalhost = 127.0.0.1"
}

// transformProxies 转换节点（完整的旧版逻辑）
func (p *ShadowrocketProducer) transformProxies(proxies []Proxy, opts *ProduceOptions) []Proxy {
